...
@frappe.whitelist()
def send_single_row_to_sap(product_name, items=None, shift_employee=''):
    log = frappe.new_doc("Sap Integration Log")
    post_product_setting = frappe.get_doc("Post Product Setting").as_dict()
    ...
    for item in items_list:
        ...
        if product.get("weight_type_sap") == "Net Weight":
            try:
                batch["Quantity"] = item["remaining_n_weight"]
            ...
            try:
                batch["Quantity"] = item["remaining_g_weight"]
...
def subscribe(client: mqtt_client, topic, measurement):
    def on_message(client, userdata, msg):
        value = re.search("[0-9]+.?[0-9]*", msg.payload.decode())
        print("value  = ", msg)
        if(value is not None):
            past_value = measurement.get('value')
            current_value = value.group()
            print("current value ", current_value)
            if( past_value and 0.5 >= abs(float(past_value) - float(current_value)) >= 0):
                # measurement['value'] = value.group()
                measurement['count'] += 1
                if(measurement['count'] == 5):
                    measurement['value'] = current_value
                    client.disconnect()
            else:
                measurement['value'] = current_value
                measurement['count'] = 0
...
def measure(scaler, queue):
    scaler = frappe.get_doc('Scaler', scaler)
    broker =  scaler.broker
    port = int(scaler.port)
    topic = scaler.topic
    client_id = 'AlQuds' + str(random.randint(20, 30))
    username = scaler.username
    password = scaler.get_password()
    username = 'AlQuds'
    measurement = {
        'count': 0
    }
    client = connect_mqtt(client_id, username, password, broker, port)
    subscribe(client, topic, measurement)
    client.loop_forever()
    response = queue.get()
    response['success'] = True
    response['value']= measurement['value']
    queue.put(response)

    
@frappe.whitelist()
def get_scaler_measurement(scaler):
    response = {}
    queue = Queue()
    queue.put(response)
    p = Process(target=measure, args=(scaler, queue))
    p.daemon = True
    p.start()
    count = 0
    ...
    while p.is_alive():
        ...
        if count > 10:
            ...
            break
    reponse = queue.get()
    reponse['success'] = reponse.get('success', False)
    return reponse